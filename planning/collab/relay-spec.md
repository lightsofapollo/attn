  ...
  "envelopes": [
    {
      ...
      "signalGeneration": 1736012345678,
...
- Broadcast cursor/view updates use `kind == "signal"` plus
  `signalClass == "presence"` over the dedicated unordered, zero-retransmit
  `attn-presence` WebRTC DataChannel. They are dropped when no direct peer
  channel is available. `POST /envelopes` rejects any V3 batch containing this
  class with `400 ATTN_PRESENCE_DIRECT_ONLY` before per-device rate, PoW replay,
  signal-generation, sequence, event-time, alarm, or latest-state mutation.
  This prevents the two-second refresh loop from consuming Durable Object
  `rows_written`. Unclassified signals, including encrypted document steps and
  WebRTC negotiation, retain the durable behavior above.
...
3. Server pushes `envelope` and socket-roster `presence` frames as they happen.
   ...
   Current clients announce their cursor/view directly when the WebRTC
   presence channel opens. Legacy latest-presence rows may be read during the
   room-lifetime cutover but no new relay presence uploads are accepted. V3
   anonymous viewers receive only non-signal `envelope` frames;
...
latest_presence_v3:<enc(participantId)>:<enc(deviceId)> -> latest signed V3 presence Envelope JSON
