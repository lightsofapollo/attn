...
- `webrtc-rs` is a large crate (transitively brings tokio, rcgen, sctp, dtls, openssl-sys or rustls). Run `cargo tree -e features --no-default-features --no-dev-dependencies` before merging to confirm the binary stays under the **40 MiB** target. If it doesn't, evaluate feature flags or revisit. **(Revised 25 → 30 → 32 MiB:** with `webrtc-rs` landed, a fully size-optimized release build — `opt-level="s"`, `lto="fat"`, `strip` — is ~29 MiB; the P2P transport stack makes 25 unrealistic, so the budget was raised to 30. It was raised again 30 → 32 when the `tracing`/`tracing-subscriber`/`time` observability stack landed for structured daemon logging (~0.96 MiB), which was deemed worth the spend for debuggability of relay/remote sync issues. The gate was later raised 32 → 40 when transparent snapshot compression landed (`flate2`, attn-ojs): the 32 MiB build passed by only 3 KB, leaving no room for any further dependency. 40 MiB buys headroom for the remaining collab roadmap without re-litigating each landing. The gate enforces 40 in `scripts/check-binary-size.sh`.)
...
**13. Browser secret persistence: memory-only by default, explicit remembered-room opt-in.** URL fragment (`#key=`) is parsed once on load and immediately stripped via `history.replaceState`. Invite-only sessions hold key material only in JS heap and persist no recoverable room key; reload requires the invite again. A user may explicitly choose **Remember this room**, which stores a non-extractable HKDF root-key capability in IndexedDB plus an identity encrypted under a root-derived local-storage key. The raw `roomSecret`, derived raw room keys, URL fragment, plaintext events, and plaintext snapshots are never persisted by this opt-in. Remembering trusts the browser profile and code served from the hosted origin to reopen the room; it does not protect against a malicious same-origin deployment. `sessionStorage`, `localStorage`, cookies, and Cache Storage remain outside the collaboration data path. Pins: `crypto-spec.md` Invite URLs; implemented by `attn-egi.3`.
