...
<section class="grid lg:grid-cols-2 gap-12 items-center py-24">
	...
	<div class="relative">
		<img
			src={lightSrc}
			{alt}
			class="w-full rounded-xl shadow-lg border border-border transition-opacity duration-500"
			class:opacity-0={getTheme() === 'dark'}
		/>
		<img
			src={darkSrc}
			{alt}
			class="absolute inset-0 w-full rounded-xl shadow-lg border border-border transition-opacity duration-500"
			class:opacity-0={getTheme() !== 'dark'}
		/>
	</div>
</section>
