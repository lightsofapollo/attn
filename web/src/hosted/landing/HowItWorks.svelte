...
<section class="chapter" id="how">
  ...
  <!--
    The guarantee stays visible, while the complete ledger is an explicit,
    native disclosure. Every line in the ledger is taken from the documented
    threat model in planning/collab/relay-spec.md §Threat Model ("The server is
    honest-but-curious. It may: … The server must not: …"), not written for
    marketing. If that spec changes, this changes with it.
  -->
  <div class="security-guarantee">
    <p><strong>End-to-end encrypted when you share.</strong> The relay routes ciphertext and never holds a key.</p>
    <details class="threat-model">
      <summary>Read the threat model</summary>
