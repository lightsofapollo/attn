...
const generatedPackDir = path.join(projectRoot, 'src/lib/vscode-icon-packs');
...
function iconFilesForMappings(mappings) {
  const files = new Set([
...
function writeGeneratedPackModule(pack, mappings) {
  const lines = [];
  ...
  iconFilesForMappings(mappings).forEach((file, idx) => {
  ...
  lines.push('');
  lines.push("import type { IconPackIcons } from '../vscode-icon-map.generated';");
  lines.push('');
  ...
  lines.push('export const ICON_PACK: IconPackIcons = {');
  lines.push(`  DEFAULT_FILE_ICON: ${importVarByFile.get(mappings.defaultFile)},`);
  lines.push(`  DEFAULT_FOLDER_ICON: ${importVarByFile.get(mappings.defaultFolder)},`);
  lines.push(`  DEFAULT_FOLDER_OPEN_ICON: ${importVarByFile.get(mappings.defaultFolderExpanded)},`);
  ...
  return writeFileIfChanged(path.join(generatedPackDir, `${pack}.generated.ts`), `${lines.join('\n')}\n`);
}

function writeGeneratedManifest(defaultPack) {
  const lines = [
    '// AUTO-GENERATED by scripts/generate-vscode-icon-map.mjs',
    '// Do not edit by hand.',
    '',
    `export type IconPack = ${SUPPORTED_PACKS.map((pack) => JSON.stringify(pack)).join(' | ')};`,
    `export const ICON_PACK_IDS = ${JSON.stringify(SUPPORTED_PACKS)} as const satisfies readonly IconPack[];`,
    `export const DEFAULT_ICON_PACK: IconPack = ${JSON.stringify(defaultPack)};`,
    '',
    ...
    '  switch (pack) {',
    ...SUPPORTED_PACKS.map((pack) => `    case ${JSON.stringify(pack)}: return (await import('./vscode-icon-packs/${pack}.generated')).ICON_PACK;`),
    '  }',
    '}',
    '',
  ];
...
function writeReadme(materialVersion, copiedIcons, defaultPack) {
  // This directory is COMMITTED (attn-6q7b), so it ships third-party SVGs.
  // Record where each one came from and under what licence, and pin the two
  // registry packs by version and the three fetched packs by commit, so the
  // provenance of every file here is auditable from the tree alone.
  const pins = ICON_PACK_PINS.map(
    (pack) => `- \`${pack.name}\` — ${pack.repo}@\`${pack.commit}\` (${pack.license})`,
  ).join('\n');
...
- \`material-icon-theme@${materialVersion}\` (MIT)
- \`seti-file-icons\` (MIT)
...
## Included SVG files (${copiedIcons.length})
...
/** Root of a fetched icon pack, with the fix if it has not been fetched. */
function iconPackRoot(name) {
  const root = path.join(projectRoot, '.icon-packs', name);
  const pin = ICON_PACK_PINS.find((pack) => pack.name === name);
  if (!fs.existsSync(path.join(root, '.commit'))) {
    throw new Error(
      `icon pack "${name}" is not fetched. Run: npm run fetch:icon-packs`,
    );
  }
  const commit = fs.readFileSync(path.join(root, '.commit'), 'utf8').trim();
  if (pin && commit !== pin.commit) {
    throw new Error(
      `icon pack "${name}" is at ${commit.slice(0, 12)}, pinned to ${pin.commit.slice(0, 12)}. Run: npm run fetch:icon-packs`,
    );
  }
  return root;
}
...
function main() {
  ...
  // These three are not on npm, so they come from `npm run fetch:icon-packs`
  // (pinned source tarballs) rather than node_modules (attn-6q7b). Material and
  // seti are ordinary registry packages and still resolve normally.
  const eyeconsRoot = iconPackRoot('eyecons');

  const catppuccinRoot = iconPackRoot('catppuccin-vsc-icons');
  const catppuccinIconsDir = path.join(catppuccinRoot, 'icons/latte');

  const vscodeIconsRoot = iconPackRoot('vscode-icons');
  ...
  let changedPackModules = 0;
  for (const pack of SUPPORTED_PACKS) {
    if (writeGeneratedPackModule(pack, packMappings[pack])) changedPackModules += 1;
  }
  const currentPackModules = new Set(SUPPORTED_PACKS.map((pack) => `${pack}.generated.ts`));
  for (const existing of fs.readdirSync(generatedPackDir)) {
    if (!currentPackModules.has(existing)) {
      fs.rmSync(path.join(generatedPackDir, existing), { force: true });
      changedPackModules += 1;
    }
  }
  const moduleChanged = writeGeneratedManifest(defaultPack);
  const readmeChanged = writeReadme(materialPkg.version ?? 'unknown', copied, defaultPack);
  const changedFiles = changedIconFiles + changedPackModules + (moduleChanged ? 1 : 0) + (readmeChanged ? 1 : 0);
